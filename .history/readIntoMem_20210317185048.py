...
卷积神经网络类：
    ...
    输入层：(1,64,64)   1指的是单通道(非RGB彩色图),两个64分别是输入数据的高和宽
...
if __name__ == '__main__':
    ...
    ****************************** '''
    ...
    # 加载检查点1
    n = 3
    checkpoint = torch.load('./checkpoint_'+str(n)+'_epoch.pkl')
    net.load_state_dict(checkpoint['model_state_dict'])
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    loss = checkpoint['loss']
    epoch = checkpoint['epoch']
    logging.info('加载检查点'+str(n)+'完毕')

    logging.info('开始从检查点'+str(n)+'训练')
    torch.set_num_threads(8)  # 多线程
    while epoch < 7:  # 不要用for循环，那样的epoch属于局部变量，无法加载检查点数据
