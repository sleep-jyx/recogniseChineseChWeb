...
if __name__ == '__main__':
    ...
    picPath = 'myCharacter/jian.png'
    inputs = Image.open(picPath)
    inputs = inputs.resize((64, 64))
    inputs.show()

    transform = transforms.Compose([transforms.Resize((64, 64)),  # 缩放为正方形；但如果只有一个数，代表最短边缩放到这个数，但形状不变即长宽等比例缩小
                                    transforms.Grayscale(),
                                    transforms.ToTensor()])
    inputs = transform(inputs)  # tensor数据格式是torch(C,H,W)
    inputs = torch.unsqueeze(inputs, dim=0)  # 增加一维(1,C,H,W)

    outputs = model(inputs)
    _, predict = torch.max(outputs.data, 1)
    with open('char_dict', 'rb') as f:
        dict = pickle.load(f)
    print('预测结果:', list(dict.keys())[
          list(dict.values()).index(predict.item())])
